...
**State — zustand + AsyncStorage**: every store in `src/store/` uses `persist(createJSONStorage(() => AsyncStorage))`. Each store key is `stok.<name>.v1` — the `.v1` suffix is part of the AsyncStorage key, not zustand's `version` field. **Don't bump the key suffix to ship a schema change** (it orphans every existing user's data). Instead, when a store's persisted shape changes incompatibly, add `version: N` + a `migrate(persistedState, version)` function to that store's persist options; zustand calls migrate transparently on rehydrate. The store-level `.v1` key suffix exists only for a deliberate, all-users wipe and should stay frozen otherwise. Stores are independent and only orchestrate each other at well-defined seams:
